 ...
 while not all(done):
  ...
  for off in range(0,len(ids),batch_cap):
   ...
   for j,i in enumerate(chunk):
    a=acts[j];ca=canonical_actions(a,states[i].player) if is_canonical(encoding) else a;v=logits[j,ca].float().cpu().numpy();temp=1 if states[i].ply<20 else .25;v=v/max(temp,.05);v-=v.max();p=np.exp(v);p/=p.sum();k=int(np.random.choice(len(a),p=p));choice=int(a[k]);pi=np.zeros(ACTION_SIZE,np.float16);pi[ca]=p.astype(np.float16);tr[i].append((encode_batch([states[i]],encoding)[0].astype(np.float16),pi,states[i].player));states[i]=apply_unchecked(states[i],choice)
